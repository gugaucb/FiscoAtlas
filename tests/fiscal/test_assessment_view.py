...
def setup(db, residente):
    ...
    with mock.patch.object(EventService, "_ptax_rate", return_value=RATE):
        ...
        EventService().record(dict(account=acct, event_type="DIVIDEND", asset=asset, trade_date=date(2026, 6, 15), quantity=Decimal(10), per_share_usd=Decimal(1), tax_usd=Decimal(1), foreign_tax_payment_date=date(2026, 6, 15), date_evidence_source="BROKER_STATEMENT", country_code="US", jurisdiction_level="FEDERAL", tax_type="WITHHOLDING_INCOME_TAX", recoverability_status="NON_RECOVERABLE"))
    return acct, asset
